...
Array<FixPoint> Conv2D::forward(const Array<FixPoint>& inputs) {
  //test
  //Array<FixPoint> test(Array<FixPoint>::pad(inputs, ((Tuple<Pair<int> >)Pair<int>(0, 0),
  //                                                                      Pair<int>(0, 0),
  //                                                                      Pair<int>(1, 1),
  //                                                                      Pair<int>(1, 1))));
...
Array<FixPoint> Conv2D::backward(const Array<FixPoint>& dout) {
  ...
  // cout << "weights:" << endl << dw.transpose(1, 2, 3, 0) << endl;
  // cout << "biases:" << endl << db << endl;
...
FixPoint SoftmaxCrossEntropy::forward(const Array<FixPoint>& pred, const Array<FixPoint>& target) {
  ...
  diff = softmax - target;
  return -((target * Array<FixPoint>(softmax + .001).log()).sum() / (FixPoint)target.shape(0));
}
...
Tuple<float> Model::fit(const Array<FixPoint>& inputs, const Array<FixPoint>& target) {
  // calulate loss
  Array<FixPoint> pred(predict(inputs));
  // cout << "Predict: " << pred.shape() << endl;
  // cout << "Except:  " << target.shape() << endl;
  // cout << "Except:  " << (pred.argmax(1) == target.argmax(1)) << endl;
  float acc((pred.argmax(1) == target.argmax(1)).sum() / (float)target.shape(0));
  float losses(loss->forward(pred, target));
